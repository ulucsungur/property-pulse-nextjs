...
const PropertyEditForm = ({ property }) => {
    ...
    // 3. OLANAKLAR LİSTESİ (Döngü için)
    const amenitiesList = [
        { value: "Wifi", key: "wifi" },
    ...
    // 4. YAPAY ZEKA FONKSİYONU (Locale eklendi)
    const generateAIDescription = async () => {
        ...
        try {
            const res = await fetch("/api/generate-description", {
                ...
                // DİL BİLGİSİNİ GÖNDERİYORUZ:
                body: JSON.stringify({ name, type, location: { city, state }, beds, baths, amenities, locale })
            });
    ...
    const handleFormSubmit = async (formData) => {
        // Silinen resimleri ekle
        deletedImages.forEach((img) => {
            formData.append("delete_images", img);
        });

        // Validation (Aynı mantık korunuyor)
    ...
    return (
        <form action={handleFormSubmit}>
            ...
            {/* --- RESİM YÖNETİMİ (EDIT'E ÖZEL) --- */}
            <div className="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg border border-gray-200 dark:border-gray-600">
                <label className="block text-gray-700 dark:text-gray-200 font-bold mb-3">{t('manageImages')}</label>
                ...
                {visibleImages.length > 0 ? (
                    ...
                    <p className="text-gray-500 dark:text-gray-300 text-sm mb-4 italic">{t('noImages')}</p>
                )}

                <label className="block text-sm text-gray-600 dark:text-gray-300 mb-2 font-bold">{t('addNewImages')}</label>
                <div className="flex items-center justify-center w-full mb-4">
                    <label htmlFor="new_images" className="flex flex-col items-center justify-center w-full h-32 border-2 border-blue-300 border-dashed rounded-lg cursor-pointer bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-600 transition">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">{t('maxImages')}</p>
                        </div>
                ...
                {newImagesPreview.length > 0 && (
                    <div className="mt-4 p-2 bg-green-50 dark:bg-green-900 rounded-md border border-green-100 dark:border-green-800">
                        <p className="text-sm text-green-700 dark:text-green-200 font-bold mb-2">{t('selectedUpload')} ({newImagesPreview.length}):</p>
            ...
            {/* TİP VE İSİM */}
            <div className="mb-4">
                <label htmlFor="type" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('type')}</label>
                <select
                    id="type"
                    name="type"
            ...
            <div className="mb-4">
                <label className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('name')}</label>
                <input
                    type="text"
                    id="name"
                    name="name"
            ...
            <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                    ...
                    >
                        {aiLoading ? <FaSpinner className="animate-spin" /> : <FaMagic />} {aiLoading ? t('loadingAI') : t('btnAI')}
                    </button>
                </div>
                <textarea
                    id="description"
            ...
            <div className="mb-4 bg-gray-50 dark:bg-gray-700 p-4 rounded-md border border-gray-200 dark:border-gray-600">
                ...
                <input type="text" id="street" name="location.street" className="border rounded w-full py-2 px-3 mb-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-white dark:border-gray-600" placeholder={t('street')} defaultValue={property.location.street} />
                <input type="text" id="city" name="location.city" className="border rounded w-full py-2 px-3 mb-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-white dark:border-gray-600" placeholder={t('city')} defaultValue={property.location.city} required />
                <input type="text" id="state" name="location.state" className="border rounded w-full py-2 px-3 mb-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-white dark:border-gray-600" placeholder={t('state')} defaultValue={property.location.state} required />
                <input type="text" id="zipcode" name="location.zipCode" className="border rounded w-full py-2 px-3 mb-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-white dark:border-gray-600" placeholder={t('zip')} defaultValue={property.location.zipCode} />
            </div>

            {/* DETAYLAR (Yatak, Banyo vb.) */}
            <div className="mb-4 flex flex-wrap">
                <div className="w-full sm:w-1/3 pr-2">
                    <label htmlFor="beds" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('beds')}</label>
                ...
                <div className="w-full sm:w-1/3 px-2">
                    <label htmlFor="baths" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('baths')}</label>
                ...
                <div className="w-full sm:w-1/3 pl-2">
                    <label htmlFor="square_feet" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('sqft')}</label>
            ...
            {/* OLANAKLAR (DÖNGÜ İLE) */}
            <div className="mb-4">
                <label className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('amenities')}</label>
                    ...
                    {amenitiesList.map((item) => (
                        <div key={item.key}>
                            <input
                                ...
                                value={item.value}
                                className="mr-2"
                                // Varsayılan olarak seçili mi? Kontrolü
                                defaultChecked={property.amenities.includes(item.value)}
                            />
                            <label htmlFor={`amenity_${item.key}`}>
                                {tAmenities(item.key)}
                            </label>
                        </div>
                    ))}
            ...
            <div className="mb-4">
                <label htmlFor="seller_email" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('sellerEmail')}</label>
            ...
            <div className="mb-4">
                <label htmlFor="seller_phone" className="block text-gray-700 dark:text-gray-200 font-bold mb-2">{t('sellerPhone')}</label>
