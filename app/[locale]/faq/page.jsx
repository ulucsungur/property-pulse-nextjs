...
// API fetch yerine doğrudan DB bağlantısı yapıyoruz
async function getFaqs() {
    try {
        await connectToDatabase();
        // Verileri "Sıralama (order)" alanına göre küçükten büyüğe diziyoruz
        const faqs = await Faq.find({}).sort({ order: 1 }).lean();
        return JSON.parse(JSON.stringify(faqs)); // Plain object dönüşümü (Next.js kuralı)
    } catch (error) {
        console.error("SSS verisi DB'den çekilemedi:", error);
        return [];
    }
}
        ...
        <section className="bg-blue-50 dark:bg-slate-950 py-16 min-h-screen transition-colors duration-300 mt-16">
            <div className="container mx-auto px-6 max-w-4xl">
                ...
                <div className="space-y-4">
                    {faqs.map((faq) => (
                        ...
                        >
                            <summary className="flex justify-between items-center p-5 cursor-pointer list-none font-semibold text-lg text-gray-800 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                {/* DÜZELTME: Admin panelindeki alt tireli (flat) yapıya göre okuyoruz */}
                    ...
                    {faqs.length === 0 && (
                        <div className="text-center text-gray-500 dark:text-slate-500 py-10 italic">
